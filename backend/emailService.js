  ...
  <table width="580" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <tr><td style="background:#F8FAFC;border-top:1px solid #E2E8F0;padding:20px 40px;text-align:center;">
      <p style="font-size:12px;color:#94A3B8;margin:0;">© 2026 Kingsley's Chicken ·
        ...
        · TechScape Solution</p>
      <p style="font-size:11px;color:#CBD5E1;margin:6px 0 0;">Automated message — do not reply directly.</p>
    </td></tr>
...
async function sendAccountActivatedEmail({ username, email, role, assignedTools = [] }) {
  const roleMap = { admin:'Admin', operation_manager:'Operation Manager', staff:'Staff Member', user:'User' };
...
async function sendAccountDeactivatedEmail({ username, email }) {
  await transporter.sendMail({
    from: `"${process.env.APP_NAME}" <${process.env.GMAIL_USER}>`,
    to: email,
...
async function sendToolsUpdatedEmail({ username, email, assignedTools = [] }) {
  await transporter.sendMail({
    from: `"${process.env.APP_NAME}" <${process.env.GMAIL_USER}>`,
    to: email,
