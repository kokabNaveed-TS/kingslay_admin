...
const COOKIE = require("../config/cookie"); // recommended
const db = require('../db');
...
// /api/admin/users/:idimpersonate
router.post("/users/:id/impersonate", async (req, res) => {
  try {
    console.log("STEP 1: ENTERED ROUTE");
...
router.post("/stop-impersonation", requireAuth, async (req, res) => {
  try {
    ...
    const original = req.user.originalUser;
